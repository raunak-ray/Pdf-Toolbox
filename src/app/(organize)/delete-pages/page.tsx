...
import { deletePdfPages } from "@/lib/pdfTools";
  ...
  return (
    <div className="flex-1 dot-pattern">
      <div className="max-w-lg md:max-w-2xl lg:max-w-5xl px-4 py-6 mx-auto">
            ...
            {Array.from({ length: totalPages }).map((_, index) => {
              ...
              return (
                <motion.div
                  ...
                  ${
                    isSelected
                      ? "border-red-500 bg-red-50 shadow-[5px_5px_0_#111]"
                      : "border-black shadow-[5px_5px_0_#111]"
                  }`}
                >
                  {/* Preview */}
                  <div
                    className={`p-2 ${isSelected ? "bg-red-200" : "bg-gray-100"}`}
                  >
        ...
        {/* Action Button (no delete logic yet) */}
        {file && selectedPages.length > 0 && (
          <motion.button
            className="mt-5 bg-red-500 font-bold text-white shadow-[5px_5px_0_#111] border-2 border-black px-4 py-2 rounded-lg cursor-pointer sticky bottom-4 right-2 w-full md:w-auto"
            whileHover={{
              x: 2,
              y: 2,
              boxShadow: "2px 2px 0 #111",
            ...
            whileTap={{ scale: 0.95 }}
            onClick={() => deletePdfPages(file.file, selectedPages)}
          >
